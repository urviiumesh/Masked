...
Face recognition became an important contactless identification tool across access control, security, attendance, and other automated systems.
Post-COVID, masks and other face coverings became common in everyday environments, creating a persistent challenge for these systems.
A practical recognition system therefore needs to identify people reliably without requiring them to remove a mask.
## Where Conventional Systems Fall Short
Most face recognition models perform best when the full face is visible and can lose useful facial information when the nose and lower face are covered.
This can lead to missed identities, lower confidence, and unreliable authentication in real-world camera feeds.
The challenge becomes greater with different mask types, partial occlusion, lighting changes, camera angles, and low-resolution video.
## Important Use Cases
- **Access Control:** Identify authorized people while they remain masked.
- **Security:** Maintain recognition capability across CCTV and live camera feeds.
- **Attendance:** Support contactless identification without requiring mask removal.
- **Public and Industrial Environments:** Recognize individuals in environments where masks, safety equipment, or other occlusions are expected.
## Overview
------
## Overview
This project provides a mask-aware face recognition pipeline built on InsightFace's `buffalo_l` ArcFace backbone.
It combines face detection, occlusion-aware enrollment, cosine-similarity matching, and controlled online adaptation.
